  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            onClick={loadHosts}
            disabled={isLoading}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-panel-hover hover:bg-panel-border border border-border text-text transition-colors disabled:opacity-50"
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white shadow-xs transition-colors"
          >
      ...
      {feedbackMessage && (
        ...
        >
          ...
          <button
            ...
            className="p-1 rounded-md hover:bg-white/10 text-text-muted hover:text-text transition-colors"
          >
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        {/* Scope Tabs */}
        <div className="flex items-center p-1 bg-panel border border-border rounded-xl">
          <button
            type="button"
        ...
        <div className="relative min-w-[220px]">
          ...
          <input
            ...
            className="w-full pl-8 pr-8 py-1.5 text-xs bg-panel border border-border rounded-xl text-text placeholder-text-muted focus:outline-hidden focus:border-indigo-500 transition-colors"
          />
          {searchQuery && (
            <button
              type="button"
      ...
      {filteredHostItems.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 text-center bg-panel/50 border border-border/60 rounded-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
          {filteredHostItems.map((item) => {
            ...
            return (
              <div
                key={hostKey}
                className="group flex flex-col justify-between p-4 bg-panel border border-border/80 rounded-2xl hover:border-border hover:shadow-xs transition-all"
              >
                <div>
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      {item.config.compat && (
                        <span className="px-1.5 py-0.5 rounded-md text-[10px] font-medium bg-amber-500/10 border border-amber-500/20 text-amber-400 shrink-0">
                    ...
                    <div className="flex items-center gap-1 opacity-80 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="p-1.5 rounded-lg hover:bg-panel-hover text-text-muted hover:text-text transition-colors"
                  ...
                  <div className="space-y-1.5 text-xs text-text-muted">
                    <div className="flex items-center gap-1.5 font-mono text-[11px] text-text">
                      ...
                      {item.config.port && item.config.port !== 22 && (
                        <span className="text-text-muted">:{item.config.port}</span>
                      )}
                    </div>
                    ...
                    {item.config.description && (
                      <p className="text-xs text-text-muted line-clamp-2 italic pt-0.5">
                        {item.config.description}
                      </p>
                    )}
                  </div>
          ...
          <div className="relative w-full max-w-lg bg-panel border border-border rounded-2xl shadow-2xl p-6 space-y-4 animate-scale-in">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-border">
              ...
              <button
                ...
                className="p-1.5 rounded-lg text-text-muted hover:text-text hover:bg-panel-hover transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleAddSubmit} className="space-y-3.5">
                  ...
                  <label
                    className={`flex items-center gap-2 p-2.5 rounded-xl border cursor-pointer transition-colors ${
                      addScope === 'project'
                        ...
                        : 'bg-panel-hover border-border text-text-muted hover:text-text'
                    }`}
                  >
                    <input
                      type="radio"
                      ...
                      className="sr-only"
                    />
                    <FolderGit2 className="w-4 h-4 shrink-0 text-indigo-400" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-text mb-1">
                    {t('ops.ssh.addModal.name')} <span className="text-rose-400">*</span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full px-3 py-1.5 text-xs bg-panel border border-border rounded-xl text-text placeholder-text-muted focus:outline-hidden focus:border-indigo-500 font-mono"
                  />
                  <p className="text-[10px] text-text-muted mt-1">{t('ops.ssh.addModal.nameHint')}</p>
                </div>

                <div>
                  <label className="block text-xs font-medium text-text mb-1">
                    {t('ops.ssh.addModal.host')} <span className="text-rose-400">*</span>
                  </label>
                  <input
                    type="text"
                    required
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 text-xs bg-panel border border-border rounded-xl text-text placeholder-text-muted focus:outline-hidden focus:border-indigo-500 font-mono"
                  />
                  <button
                    type="button"
                    onClick={handleBrowseKeyFile}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-xl bg-panel-hover hover:bg-panel-border border border-border text-text transition-colors shrink-0"
                  >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 text-xs bg-panel border border-border rounded-xl text-text placeholder-text-muted focus:outline-hidden focus:border-indigo-500"
                />
              </div>
              ...
              <div className="flex items-start gap-2.5 pt-1">
                ...
                />
                <label htmlFor="sshCompatCheckbox" className="text-xs cursor-pointer">
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-border">
                <button
                  ...
                  onClick={() => setShowAddModal(false)}
                  disabled={isAdding}
                  className="px-3 py-1.5 text-xs font-medium rounded-xl bg-panel hover:bg-panel-hover border border-border text-text transition-colors"
                >
                  {t('common.cancel')}
                </button>
                <button
